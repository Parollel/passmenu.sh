...
_usage() {
	...
	printf "Usage: passmenu_enhanced.sh [options]\n"
...
_version() {
	printf "passmenu_enhanced.sh ${PROGRAM_VERSION}\n"
	exit
}

args="$(getopt --options hm:t:V --longoptions help,menu:,type:,usage,version -n 'passmenu_enhanced.sh' -- "$@")"
eval set -- "${args}"
while true; do
	case "${1}" in
